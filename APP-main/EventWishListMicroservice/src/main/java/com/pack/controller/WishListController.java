...
public class WishListController {
	...
	@GetMapping("/viewFavourite/{eventId}/{userId}")
	public ResponseEntity<Object> viewFavouriteEvent(@RequestHeader(name = "Authorization", required = true) String token,@PathVariable long eventId,@PathVariable int userId) throws WishListEventNotFoundException {
		logger.info("requesting list of events {} {}",userId,eventId);
		WishList wishList =wishListService.getWishListEvent(eventId,userId);
		logger.info("Event data retrieved successfully!");
		return ResponseHandler.generateResponse("Event data retrieved successfully!", HttpStatus.OK, wishList);
